...
// (values that each key in the map points to, in string format). If the path is not
// provided in the map, then the fallback http.Handler will be called instead.
...
// also implements http.Handler) that will attempt to map any paths to their corresponding URL.
